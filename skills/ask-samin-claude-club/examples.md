# Reply examples

## Good (pass) — top hit + Related from search

> Here's where Samin covers this in Claude Club: [📝 Give the bot eyes + a simple strategy](https://www.skool.com/claude/classroom/e63905c6?md=4ca72ef56dd8419f976bea5968b3de15).
>
> This is Samin’s approach in Claude Club (Claude Masterclass → Day 15 — Give the Bot a Strategy).
>
> Copy “smart money” via CapitalTrades into an Alpaca **paper** account — education only.
>
> Related: [🔗 Resources](https://www.skool.com/claude/classroom/e63905c6?md=…) — prompts and links for the same day.
>
> Related: [Other matching Claude Club page](https://www.skool.com/claude/classroom/…) — only if search returned another strong hit (including another course).

## Bad (fail)

> Here's the lesson: 📝 Give the bot eyes + a simple strategy — Day 15
>
> *(Title only — no `https://www.skool.com/...` — fails Claude Club CTA.)*

> Related: 📂 Day 16 — Run the Wheel Strategy while you sleep
>
> *(Related without a clickable Skool URL — also fail.)*
