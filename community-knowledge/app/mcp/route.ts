...
import { getRequestIdentity, jsonResponse } from "@/src/http";
import { buildMcpFetchDocument, buildMcpSearchResults, flattenCurriculumNodes } from "@/src/mcp/evidence";
...
const handler = createMcpHandler(
  (server) => {
    server.registerTool(
      "search",
      {
        ...
        description:
          "Hybrid search over published lessons, posts, and transcripts. Returns evidence chunks with canonical Skool URLs.",
        inputSchema: {
  ...
  { serverInfo: { name: "community-knowledge", version: "0.1.0" } },
